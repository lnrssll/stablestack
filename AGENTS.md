...
| Typed queries | TypeSQL (`typesql-cli`) | `src/sql/` → generates `src/db/queries/` |
...
src/
  ...
  db/
    ...
    queries/         # !! GENERATED by TypeSQL — never edit by hand
...
This is a three-step workflow: migrate → compile → import.
...
Compile (or let the watcher pick it up):
```bash
npx typesql compile
```
This generates typed modules in `src/db/queries/`. Do not edit them.

**Step 3 — use the generated functions in a route:**
```ts
